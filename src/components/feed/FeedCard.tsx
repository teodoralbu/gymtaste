      ...
      <Link href={`/flavors/${ratingData.flavor?.slug ?? ''}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block', padding: '0 16px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minHeight: '52px' }}>
          ...
          <div style={{
            ...
            {product?.image_url ? (
              <Image src={product.image_url} alt={`${product?.name ?? 'Product'} by ${brand?.name ?? 'Unknown'}`} width={72} height={72} style={{ objectFit: 'cover', objectPosition: 'center 65%', width: '100%', height: '100%' }} />
            ) : (
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
