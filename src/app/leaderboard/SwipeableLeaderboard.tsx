...
export function SwipeableLeaderboard({ allData }: Props) {
  ...
  // Use a non-passive touchmove listener so e.preventDefault() actually works in React 18.
  // Without this, React attaches passive handlers by default and horizontal swipes also
  // trigger page scroll simultaneously, causing the "glitch" when scrolling.
  useEffect(() => {
    const el = swipeRailRef.current
    if (!el) return
  ...
  return (
    <div>
      {/* ── Tab pills — sticky, lives outside the overflow-clipped swipe rail ── */}
      <div
      ...
      {/* ── Overflow clip wrapper — isolates the swipe rail's negative margins
           so they don't cause body horizontal overflow. Sticky elements must be
           OUTSIDE this wrapper (overflow-x:hidden creates a scroll container). ── */}
      <div style={{ overflowX: 'hidden' }}>
        {/* Swipe rail — negative margins push it to full viewport width */}
        <div
          ref={swipeRailRef}
          style={{ overflow: 'hidden', touchAction: 'pan-y', margin: '0 -16px' }}
